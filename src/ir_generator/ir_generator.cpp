...
if (elementType->isIntegerTy(32)) baseType = SymbolType::Integer;
else if (elementType->isFloatTy()) baseType = SymbolType::Float;
else if (elementType->isIntegerTy(1)) baseType = SymbolType::Boolean;
else baseType = SymbolType::Integer; // Default

// Set output parameters
if (outBaseType) *outBaseType = baseType;
if (outDimensions) *outDimensions = totalDimensions;
      ...
      if (auto assign = dynamic_cast<AssignmentExpression*>(stmt)) {
          if (auto arrayLit = dynamic_cast<ArrayLiteral*>(assign->value)) {
              ...
              if (auto id = dynamic_cast<Identifier*>(assign->assignee)) {
                  if (auto entry = findSymbol(id->name)) {
  ...
  // Handle VariableInitialization (single variable with initializer)
    ...
    if (auto arrayLit = dynamic_cast<ArrayLiteral*>(varInit->initializer)) {
        ...
        if (dimensions == 1 && isUniformArray(arrayLit)) {
            ...
            ty = PointerType::getUnqual(arrType); // Store as pointer to array
        }
        else {
            ArrayType* arrType = inferArrayType(arrayLit);
            init = createNestedArray(arrayLit, arrType);
        }
    }
    else {
    ...
    // Update symbol table with correct type information
...
Value* IRGenerator::codegenVariableInitialization(VariableInitialization* init) {
  ...
  if (auto arrayLit = dynamic_cast<ArrayLiteral*>(init->initializer)) {
      ...
      Value* jagged = createJaggedArray(arrayLit, declareMalloc(), 
                                      &actualBaseType, &actualDimensions);
  ...
  // Store complete type information
  symbolTable.top()[init->name->name] = {
      ty,         // LLVM type
      alloca,     // Alloca instruction
      baseType,   // Our base type
      dimensions  // Array dimensions
  };
...
Value* IRGenerator::codegenAssignment(AssignmentExpression* assign) {
  ...
  if (value->getType()->isArrayTy()) {
      DataLayout dl = module->getDataLayout();
      uint64_t size = dl.getTypeAllocSize(value->getType());

      AllocaInst* tmp = builder.CreateAlloca(value->getType(), nullptr, "tmp.array");
      builder.CreateStore(value, tmp);

      Value* dst = builder.CreateBitCast(target, builder.getPtrTy());
      Value* src = builder.CreateBitCast(tmp, builder.getPtrTy());

      builder.CreateMemCpy(dst, Align(4), src, Align(4),
