...
namespace rbn::ast
{
  ...
  map<core::TokenType, string> AstNode::DataTypes = {
      {core::TokenType::INTEGER_TY, "integer"},
      {core::TokenType::BOOLEAN_TY, "boolean"},
      {core::TokenType::STRING_TY, "string"},
      {core::TokenType::FLOAT_TY, "float"},
  };

  bool AstNode::is_data_type(core::TokenType type)
  {
    return DataTypes.find(type) != DataTypes.end();
  }

  bool AstNode::is_return_type(core::TokenType type)
  {
    return is_data_type(type) || type == core::TokenType::VOID_TY;
  }
