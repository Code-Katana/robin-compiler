...
namespace rbn::ir
{
  ...
  Type *IRGenerator::getLLVMType(core::SymbolType type, int dim)
  {
    Type *baseType = nullptr;
    switch (type)
    {
    case core::SymbolType::Integer:
      baseType = Type::getInt32Ty(context);
      break;
    case core::SymbolType::Float:
      baseType = Type::getFloatTy(context);
      break;
    case core::SymbolType::Boolean:
      baseType = Type::getInt1Ty(context);
      break;
    case core::SymbolType::String:
      baseType = PointerType::getUnqual(Type::getInt8Ty(context));
      break;
    case core::SymbolType::Void:
  ...
  {
    if (auto decl = dynamic_cast<ast::VariableDefinition *>(node))
      return generate_variable_definition(decl);
    if (auto expr = dynamic_cast<ast::Expression *>(node))
      return generate_expression(expr);
    if (auto stmt = dynamic_cast<ast::Statement *>(node))
      return generate_statement(stmt);
    return nullptr;
  }

  Value *IRGenerator::generate_program(ast::ProgramDefinition *program)
  {
    ...
    for (auto *G : program->globals)
    {
      auto *initStmt = dynamic_cast<ast::VariableInitialization *>(G->def);
      if (!initStmt)
        continue;

      const string &name = initStmt->name->name;
      if (auto *lit = dynamic_cast<ast::ArrayLiteral *>(initStmt->initializer))
      {
  ...
  Value *IRGenerator::generate_function(ast::FunctionDefinition *func)
  {
    SmallVector<Type *, 8> paramTypes;
    SmallVector<ast::VariableDeclaration *, 8> paramDecls;
    SmallVector<ast::VariableInitialization *, 8> paramInits;

    for (auto *paramDef : func->parameters)
    {
      if (auto *decl = dynamic_cast<ast::VariableDeclaration *>(paramDef->def))
      {
        Type *ty = getLLVMType(
            core::Symbol::get_datatype(decl->datatype),
            core::Symbol::get_dimension(decl->datatype));
        paramTypes.push_back(ty);
        paramDecls.push_back(decl);
        paramInits.push_back(nullptr);
      }
      else if (auto *init = dynamic_cast<ast::VariableInitialization *>(paramDef->def))
      {
        Type *ty = getLLVMType(
            core::Symbol::get_datatype(init->datatype),
            core::Symbol::get_dimension(init->datatype));
        paramTypes.push_back(ty);
    ...
    Type *retTy = getLLVMType(
        core::Symbol::get_datatype(func->return_type->return_type),
        core::Symbol::get_dimension(func->return_type->return_type));
    ...
    for (auto &Arg : F->args())
    {
      ...
      if (paramDecls[idx])
      {
        ...
        baseType = core::Symbol::get_datatype(paramDecls[idx]->datatype);
        dimensions = core::Symbol::get_dimension(paramDecls[idx]->datatype);
      }
      else
      {
        auto *init = paramInits[idx];
        name = init->name->name;
        baseType = core::Symbol::get_datatype(init->datatype);
        dimensions = core::Symbol::get_dimension(init->datatype);
      }
  ...
  Value *IRGenerator::generate_statement(ast::Statement *stmt)
  {
    if (auto decl = dynamic_cast<ast::VariableDefinition *>(stmt))
      return generate_variable_definition(decl);
    if (auto assign = dynamic_cast<ast::AssignmentExpression *>(stmt))
      return generate_assignment(assign);
    if (auto ifStmt = dynamic_cast<ast::IfStatement *>(stmt))
      return generate_if_statement(ifStmt);
    if (auto whileLoop = dynamic_cast<ast::WhileLoop *>(stmt))
      return generate_while_loop(whileLoop);
    if (auto forLoop = dynamic_cast<ast::ForLoop *>(stmt))
      return generate_for_loop(forLoop);
    if (auto writeStmt = dynamic_cast<ast::WriteStatement *>(stmt))
      return generate_write_statement(writeStmt);
    if (auto readStmt = dynamic_cast<ast::ReadStatement *>(stmt))
    ...
    if (auto skip = dynamic_cast<ast::SkipStatement *>(stmt))
      return generate_skip_statement(skip);
    if (auto stop = dynamic_cast<ast::StopStatement *>(stmt))
      return generate_stop_statement(stop);

    return generate_expression(dynamic_cast<ast::Expression *>(stmt));
  }

  Value *IRGenerator::generate_skip_statement(ast::SkipStatement *stmt)
  {
    return nullptr;
  }

  Value *IRGenerator::generate_stop_statement(ast::StopStatement *stmt)
  {
  ...
  Value *IRGenerator::generate_if_statement(ast::IfStatement *ifStmt)
  {
    ...
    pushScope();
    for (ast::Statement *stmt : ifStmt->consequent)
      generate_node(stmt);
    popScope();
    ...
    if (elseBB)
    {
      ...
      pushScope();
      for (ast::Statement *stmt : ifStmt->alternate)
        generate_node(stmt);
      popScope();
      builder.CreateBr(mergeBB);
    }
  ...
  Value *IRGenerator::generate_while_loop(ast::WhileLoop *loop)
  {
    Function *func = builder.GetInsertBlock()->getParent();
    ...
    pushScope();
    for (ast::Statement *stmt : loop->body)
      generate_node(stmt);
    popScope();
    builder.CreateBr(condBB);
  ...
  Value *IRGenerator::generate_for_loop(ast::ForLoop *forLoop)
  {
    pushScope();

    if (auto assign = dynamic_cast<ast::AssignmentExpression *>(forLoop->init))
    {
      if (auto id = dynamic_cast<ast::Identifier *>(assign->assignee))
      {
    ...
    builder.SetInsertPoint(bodyBB);
    pushScope();
    for (ast::Statement *stmt : forLoop->body)
      generate_node(stmt);
    popScope();
  ...
  Value *IRGenerator::generate_return_statement(ast::ReturnStatement *retStmt)
  {
  ...
  Value *IRGenerator::generate_write_statement(ast::WriteStatement *write)
  {
  ...
  Value *IRGenerator::generate_read_statement(ast::ReadStatement *read)
  {
    ...
    {
      Type *baseType;
      Value *addr = generate_identifier_address(dynamic_cast<ast::Identifier *>(var));
      if (!addr)
      {
  ...
  Value *IRGenerator::generate_variable_definition(ast::VariableDefinition *def)
  {
    if (auto decl = dynamic_cast<ast::VariableDeclaration *>(def->def))
      return generate_variable_declaration(decl);
    if (auto init = dynamic_cast<ast::VariableInitialization *>(def->def))
      return generate_variable_initialization(init);
    return nullptr;
  }

  void IRGenerator::generate_global_variable(ast::VariableDefinition *dif)
  {
    if (auto varDecl = dynamic_cast<ast::VariableDeclaration *>(dif->def))
    {
      core::SymbolType baseType = core::Symbol::get_datatype(varDecl->datatype);
      int dimensions = core::Symbol::get_dimension(varDecl->datatype);
    ...
    else if (auto varInit = dynamic_cast<ast::VariableInitialization *>(dif->def))
    {
      core::SymbolType baseType = core::Symbol::get_datatype(varInit->datatype);
      int dimensions = core::Symbol::get_dimension(varInit->datatype);
      Type *ty = getLLVMType(baseType, dimensions);
      ...
      if (auto arrayLit = dynamic_cast<ast::ArrayLiteral *>(varInit->initializer))
      {
  ...
  Value *IRGenerator::generate_variable_initialization(ast::VariableInitialization *init)
  {
    core::SymbolType baseType = core::Symbol::get_datatype(init->datatype);
    int dimensions = core::Symbol::get_dimension(init->datatype);
    Type *ty = getLLVMType(baseType, dimensions);
    ...
    if (auto arrayLit = dynamic_cast<ast::ArrayLiteral *>(init->initializer))
    {
      core::SymbolType actualBaseType;
      ...
      if (lengthAlloca)
      {
        symbolTable.top()[varName + "_len"] = {
            Type::getInt32Ty(context), lengthAlloca, core::SymbolType::Integer, 0, 0, nullptr};
      }
    }
  ...
  Value *IRGenerator::generate_variable_declaration(ast::VariableDeclaration *decl)
  {
    core::SymbolType baseType = core::Symbol::get_datatype(decl->datatype);
    int dimensions = core::Symbol::get_dimension(decl->datatype);
    Type *ty = getLLVMType(baseType, dimensions);
  ...
  Value *IRGenerator::generate_expression(ast::Expression *expr)
  {
    if (auto index = dynamic_cast<ast::IndexExpression *>(expr))
    {
    ...
    if (auto decl = dynamic_cast<ast::VariableDefinition *>(expr))
      return generate_variable_definition(decl);
    ...
    if (auto call = dynamic_cast<ast::CallFunctionExpression *>(expr))
      return generate_call(call);

    if (auto add = dynamic_cast<ast::AdditiveExpression *>(expr))
      return generate_additive_expr(add);
    if (auto mult = dynamic_cast<ast::MultiplicativeExpression *>(expr))
    ...
    // Boolean operations
    if (auto OR = dynamic_cast<ast::OrExpression *>(expr))
      return generate_or_expr(OR);
    if (auto AND = dynamic_cast<ast::AndExpression *>(expr))
      return generate_and_expr(AND);
    if (auto eq = dynamic_cast<ast::EqualityExpression *>(expr))
      return generate_equality_expr(eq);
    if (auto re = dynamic_cast<ast::RelationalExpression *>(expr))
      return generate_relational_expr(re);

    return nullptr;
  }

  Value *IRGenerator::generate_assignment(ast::AssignmentExpression *assign)
  {
    ...
    else if (auto index = dynamic_cast<ast::IndexExpression *>(assign->assignee))
    {
    ...
    if (value->getType()->isPointerTy() && !varName.empty())
    {
      ...
      if (auto arrayLit = dynamic_cast<ast::ArrayLiteral *>(assign->value))
      {
        arraySize = arrayLit->elements.size();
  ...
  Value *IRGenerator::generate_identifier(ast::Identifier *id)
  {
    auto entryOpt = findSymbol(id->name);
    if (!entryOpt)
      return nullptr;
  ...
  Value *IRGenerator::generate_literal(ast::Literal *lit)
  {
    if (auto intLit = dynamic_cast<ast::IntegerLiteral *>(lit))
    {
      return ConstantInt::get(context, APInt(32, intLit->value));
    }
    if (auto floatLit = dynamic_cast<ast::FloatLiteral *>(lit))
    {
      return ConstantFP::get(context, APFloat(floatLit->value));
    }
    if (auto boolLit = dynamic_cast<ast::BooleanLiteral *>(lit))
    {
      return ConstantInt::get(context, APInt(1, boolLit->value));
    }
    if (auto stringLit = dynamic_cast<ast::StringLiteral *>(lit))
    {
  ...
  Value *IRGenerator::generate_or_expr(ast::OrExpression *expr)
  {
  ...
  Value *IRGenerator::generate_and_expr(ast::AndExpression *expr)
  {
    Value *L = generate_node(expr->left);
    Value *R = generate_node(expr->right);
    if (!L || !R)
  ...
  Value *IRGenerator::generate_relational_expr(ast::RelationalExpression *expr)
  {
  ...
  Value *IRGenerator::generate_equality_expr(ast::EqualityExpression *expr)
  {
    Value *L = generate_node(expr->left);
    Value *R = generate_node(expr->right);
    if (!L || !R)
  ...
  Value *IRGenerator::generate_additive_expr(ast::AdditiveExpression *expr)
  {
    Value *L = generate_expression(expr->left);
    Value *R = generate_expression(expr->right);
  ...
  Value *IRGenerator::generate_multiplicative_expr(ast::MultiplicativeExpression *expr)
  {
    Value *L = generate_expression(expr->left);
    Value *R = generate_expression(expr->right);
  ...
  Value *IRGenerator::generate_unary_expr(ast::UnaryExpression *expr)
  {
    ...
    if (expr->optr == "#")
    {
      if (auto *id = dynamic_cast<ast::Identifier *>(expr->operand))
      {
        auto symOpt = findSymbol(id->name);
        if (symOpt &&
            symOpt->baseType == core::SymbolType::String &&
            symOpt->dimensions == 0)
        {
      ...
      int numIndices = 0;
      ast::Identifier *rootId = nullptr;
      ast::Expression *cur = expr->operand;
      while (auto *ix = dynamic_cast<ast::IndexExpression *>(cur))
      {
        numIndices++;
        cur = ix->base;
      }
      rootId = dynamic_cast<ast::Identifier *>(cur);
      if (!rootId)
      {
      ...
      if (numIndices > 0)
      {
        slotAddr = generate_index_expression(static_cast<ast::IndexExpression *>(expr->operand), &dummyTy);
      }
      else
      {
      ...
      if (numIndices > 0 ||
          dynamic_cast<ast::IndexExpression *>(expr->operand))
      {
        auto sym = *findSymbol(rootId->name);
    ...
    {
      auto id = dynamic_cast<ast::Identifier *>(expr->operand);
    ...
    if (expr->optr == "@")
    {
      ...
      {
        FunctionCallee roundFunc = Intrinsic::getDeclaration(module.get(), Intrinsic::round, {ty});
  ...
  Value *IRGenerator::generate_call(ast::CallFunctionExpression *call)
  {
    ...
    ast::FunctionDefinition *funcDef = nullptr;
    ...
    for (size_t i = 0; i < numParams; ++i)
    {
      ...
      else
      {
        ...
        if (auto init = dynamic_cast<ast::VariableInitialization *>(paramDef))
        {
  ...
  Value *IRGenerator::generate_index_expression(ast::IndexExpression *expr, Type **outElementType)
  {
    std::vector<Value *> indices;
    const ast::Identifier *rootId = nullptr;
    core::SymbolType baseType;
    int totalDims;

    ast::IndexExpression *current = expr;
    while (current)
    {
      if (auto id = dynamic_cast<ast::Identifier *>(current->base))
      {
        rootId = id;
        break;
      }
      else if (auto innerExpr = dynamic_cast<ast::IndexExpression *>(current->base))
      {
    ...
    {
      indices.push_back(generate_node(current->index));
      current = dynamic_cast<ast::IndexExpression *>(current->base);
    }
  ...
  bool IRGenerator::isUniformArray(ast::ArrayLiteral *lit)
  {

    if (lit->elements.empty())
      return true;

    bool isNested = dynamic_cast<ast::ArrayLiteral *>(lit->elements[0]) != nullptr;
    size_t expectedSize = isNested ? dynamic_cast<ast::ArrayLiteral *>(lit->elements[0])->elements.size() : 0;
    Type *expectedType = generate_node(lit->elements[0])->getType();

    for (auto elem : lit->elements)
    {
      auto sub = dynamic_cast<ast::ArrayLiteral *>(elem);
  ...
  Type *IRGenerator::getElementType(ast::ArrayLiteral *lit, int *outDim)
  {
    int dimensions = 0;
    ast::ArrayLiteral *current = lit;
    ...
    while (true)
    {
      ...
      if (auto sub = dynamic_cast<ast::ArrayLiteral *>(first))
      {
        current = sub;
      }
      else
  ...
  Value *IRGenerator::createJaggedArray(ast::ArrayLiteral *lit, Function *mallocFn, core::SymbolType *outBaseType, int *outDimensions)
  {
    ...
    if (elementType->isIntegerTy(32))
      baseType = core::SymbolType::Integer;
    else if (elementType->isFloatTy())
      baseType = core::SymbolType::Float;
    else if (elementType->isIntegerTy(1))
      baseType = core::SymbolType::Boolean;
    else
      baseType = core::SymbolType::Integer;
  ...
  Constant *IRGenerator::createNestedArray(ast::ArrayLiteral *lit, ArrayType *arrType)
  {
    ...
    {
      if (auto *subLit = dynamic_cast<ast::ArrayLiteral *>(e))
      {
  ...
  ArrayType *IRGenerator::inferArrayType(ast::ArrayLiteral *lit)
  {
    std::vector<uint64_t> dims;
    ast::ArrayLiteral *current = lit;
    while (dynamic_cast<ast::ArrayLiteral *>(current->elements[0]))
    {
      dims.push_back(current->elements.size());
      current = dynamic_cast<ast::ArrayLiteral *>(current->elements[0]);
    }
    dims.push_back(current->elements.size());
  ...
  Value *IRGenerator::generate_identifier_address(ast::Identifier *id)
  {
  ...
  Value *IRGenerator::generate_l_value(ast::Expression *expr)
  {
    if (auto id = dynamic_cast<ast::Identifier *>(expr))
    {
  ...
  Value *IRGenerator::createJaggedArrayHelper(ast::ArrayLiteral *lit, Function *mallocFn, int remainingDims, Type *elementType)
  {
    ...
    for (unsigned i = 0; i < count; ++i)
    {
      ...
      {
        auto *subLit = dynamic_cast<ast::ArrayLiteral *>(lit->elements[i]);
  ...
  bool IRGenerator::isSingleDimensionArray(const SymbolEntry &entry)
  {
    return entry.dimensions == 1 &&
           entry.baseType != core::SymbolType::String;
  }
}